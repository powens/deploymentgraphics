import type { DeploymentConfig } from "../types.js";

/**
 * The six standard Warhammer 40k mission deployment maps. Each defines
 * the attacker/defender zone polygons and the five objective markers.
 *
 * Transcribed from `static/data/deployment/*.yml`; `presets.test.ts`
 * asserts the two stay in sync.
 */

export const dawnOfWar: DeploymentConfig = {
  name: "Dawn of War",
  home_edge: "long",
  attacker: {
    deployment_zone: [
      [0, 0],
      [0, 12],
      [60, 12],
      [60, 0],
    ],
  },
  defender: {
    deployment_zone: [
      [0, 32],
      [0, 44],
      [60, 44],
      [60, 32],
    ],
  },
  objectives: [
    [30, 22],
    [10, 22],
    [50, 22],
    [30, 38],
    [30, 6],
  ],
};

export const crucibleOfBattle: DeploymentConfig = {
  name: "Crucible of Battle",
  home_edge: "short",
  attacker: {
    deployment_zone: [
      [60, 0],
      [60, 44],
      [30, 0],
    ],
  },
  defender: {
    deployment_zone: [
      [30, 44],
      [0, 44],
      [0, 0],
    ],
  },
  objectives: [
    [30, 22],
    [40, 8],
    [46, 34],
    [20, 36],
    [14, 10],
  ],
};

export const hammerAndAnvil: DeploymentConfig = {
  name: "Hammer and Anvil",
  home_edge: "short",
  attacker: {
    deployment_zone: [
      [60, 0],
      [42, 0],
      [42, 44],
      [60, 44],
    ],
  },
  defender: {
    deployment_zone: [
      [18, 0],
      [0, 0],
      [0, 44],
      [18, 44],
    ],
  },
  objectives: [
    [30, 22],
    [30, 38],
    [30, 6],
    [50, 22],
    [10, 22],
  ],
};

export const searchAndDestroy: DeploymentConfig = {
  name: "Search and Destroy",
  home_edge: "long",
  attacker: {
    mask_center: 9,
    deployment_zone: [
      [60, 0],
      [60, 22],
      [30, 22],
      [30, 0],
    ],
  },
  defender: {
    mask_center: 9,
    deployment_zone: [
      [30, 22],
      [30, 44],
      [0, 44],
      [0, 22],
    ],
  },
  objectives: [
    [30, 22],
    [14, 10],
    [14, 34],
    [46, 10],
    [46, 34],
  ],
};

export const sweepingEngagement: DeploymentConfig = {
  name: "Sweeping Engagement",
  home_edge: "long",
  attacker: {
    deployment_zone: [
      [60, 44],
      [0, 44],
      [0, 30],
      [30, 30],
      [30, 36],
      [60, 36],
    ],
  },
  defender: {
    deployment_zone: [
      [0, 0],
      [60, 0],
      [60, 14],
      [30, 14],
      [30, 8],
      [0, 8],
    ],
  },
  objectives: [
    [30, 22],
    [10, 18],
    [18, 38],
    [42, 6],
    [50, 26],
  ],
};

export const tippingPoint: DeploymentConfig = {
  name: "Tipping Point",
  home_edge: "short",
  attacker: {
    deployment_zone: [
      [0, 44],
      [0, 0],
      [12, 0],
      [12, 22],
      [20, 22],
      [20, 32],
      [20, 44],
    ],
  },
  defender: {
    deployment_zone: [
      [60, 0],
      [60, 44],
      [48, 44],
      [48, 22],
      [40, 22],
      [40, 0],
    ],
  },
  objectives: [
    [30, 22],
    [22, 8],
    [14, 34],
    [38, 36],
    [46, 10],
  ],
};

/**
 * All built-in missions keyed by id. The id matches the deployment YAML
 * filename stem and the mission selector value in the browser app.
 */
export const missions = {
  dawn_of_war: dawnOfWar,
  crucible_of_battle: crucibleOfBattle,
  hammer_and_anvil: hammerAndAnvil,
  search_and_destroy: searchAndDestroy,
  sweeping_engagement: sweepingEngagement,
  tipping_point: tippingPoint,
} satisfies Record<string, DeploymentConfig>;

/** The id of a built-in mission. */
export type MissionId = keyof typeof missions;
